(defparameter *counter3* 0)
  (defun three-color-guesser (board colors SCSA last-response)
    (declare (ignore SCSA))
    (let ((previous-guess ()))
    (when (equal last-response nil) (progn (setf *bull-check* 1) (setf *counter2* 0) (setf *counter3* 0) (setf *guess* ()) (setf *check* 0) (setf *bull-count* 0) (setf *color-freq* ()) (setf *not-in-answer* ()) (setf *in-answer* ()))) ;;reset everything after each round
    ...
    (cond ((= *check* 0) *guess*)
      ...
      ((and (= *check* 2) (= *bull-check* (first last-response))) (progn (incf *bull-check*) (incf *counter2*) (setf *counter3* 0) (setf (nth *counter2* *guess*) (*counter3* *in-answer*))))
      ((and (= *check* 2) (not (= *bull-check* (first last-response)))) (incf *counter3*) (setf (nth *counter2* *guess*) (*counter3* *in-answer*)))
      (t nil))
